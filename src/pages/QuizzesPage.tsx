...
export function QuizzesPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {!loading && quizzes.length === 0 ? (
        <div className="card">
          <EmptyState
            ...
            description={canManage ? 'Create a quiz to get started.' : 'Quizzes will appear here once created.'}
            action={canManage ? <button className="btn-primary btn-md" onClick={() => setCreateOpen(true)}><Plus size={15} /> New Quiz</button> : undefined}
          />
        </div>
      ) : (
        <div className="space-y-8">
          {openQuizzes.length > 0 && (
            <QuizSection title="Open Quizzes" accent="bg-mint-500" quizzes={openQuizzes} sessions={sessions}
              scores={scores} myScore={myScore} canManage={canManage} isMember={role === 'member'}
              onEdit={setEditQuiz} onDelete={handleDelete} onGrade={setGradingQuizId} />
          )}
          {lockedQuizzes.length > 0 && (
            <QuizSection title="Scheduled — Not Open Yet" accent="bg-amber-400" quizzes={lockedQuizzes} sessions={sessions}
              scores={scores} myScore={myScore} canManage={canManage} isMember={role === 'member'}
              onEdit={setEditQuiz} onDelete={handleDelete} onGrade={setGradingQuizId} />
          )}
          {closedQuizzes.length > 0 && (
            <QuizSection title="Closed Quizzes" accent="bg-ink-300" quizzes={closedQuizzes} sessions={sessions}
              scores={scores} myScore={myScore} canManage={canManage} isMember={role === 'member'}
              onEdit={setEditQuiz} onDelete={handleDelete} onGrade={setGradingQuizId} />
          )}
        </div>
      ...
      {gradingQuizId && (
        <GradingModal quizId={gradingQuizId} onClose={() => setGradingQuizId(null)} onSaved={() => setGradingQuizId(null)} />
      )}
    </div>
  );
}
...
interface QuizSectionProps {
  title: string; accent: string; quizzes: Quiz[]; sessions: Session[];
  scores: QuizScore[]; myScore: (id: string) => QuizScore | undefined;
  canManage: boolean; isMember: boolean;
  onEdit: (q: Quiz) => void; onDelete: (id: string) => void; onGrade: (id: string) => void;
}

function QuizSection({ title, accent, quizzes, sessions, scores, myScore, canManage, isMember, onEdit, onDelete, onGrade }: QuizSectionProps) {
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {quizzes.map((qz) => (
          <QuizCard key={qz.id} quiz={qz} session={sessions.find((s) => s.id === qz.session_id)}
            mine={myScore(qz.id)} gradedCount={scores.filter((s) => s.quiz_id === qz.id).length}
            canManage={canManage} isMember={isMember}
            onEdit={() => onEdit(qz)} onDelete={() => onDelete(qz.id)} onGrade={() => onGrade(qz.id)} />
        ))}
      </div>
    </div>
  );
...
interface QuizCardProps {
  quiz: Quiz; session?: Session; mine?: QuizScore; gradedCount: number;
  canManage: boolean; isMember: boolean;
  onEdit: () => void; onDelete: () => void; onGrade: () => void;
}

function QuizCard({ quiz, session, mine, gradedCount, canManage, isMember, onEdit, onDelete, onGrade }: QuizCardProps) {
  ...
  return (
    <div className="card card-hover flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1 gap-0">
        ...
        {session && <p className="text-xs text-ink-400 mt-1 flex items-center gap-1"><ChevronRight size={10} />{session.title}</p>}
        <div className="mt-3 space-y-1 text-xs">
          {quiz.start_time && !open && (
            <p className="text-amber-600 font-medium flex items-center gap-1">
              <Lock size={11} />Opens {new Date(quiz.start_time).toLocaleString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </p>
          )}
          {quiz.start_time && open && (
            <p className="text-mint-600 flex items-center gap-1">
              <CheckCircle2 size={11} />Opened {new Date(quiz.start_time).toLocaleString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </p>
          )}
          <p className="text-ink-500 flex items-center gap-1"><CalendarDays size={11} />Closes {formatDate(quiz.deadline, { dateStyle: 'medium' })}</p>
        </div>
        {isMember && (
          <div className="mt-3 pt-3 border-t border-ink-100">
            {mine != null ? (
              <p className="text-xs font-medium text-ink-700">
                Your grade: <span className="font-semibold text-mint-600">{total}</span>
                {Number(mine.bonus) > 0 && <span className="text-ink-400 ml-1">(+{mine.bonus} bonus)</span>}
              </p>
            ) : (
              <p className="text-xs text-ink-400 flex items-center gap-1"><AlertCircle size={11} /> Not graded yet</p>
            )}
          </div>
        )}
        <div className="flex items-center gap-2 mt-4 pt-3 border-t border-ink-100">
          {quiz.form_url && open && !overdue ? (
            ...
            <button disabled className="btn-secondary btn-sm flex-1 justify-center opacity-60 cursor-not-allowed"><Lock size={13} /> Not Open Yet</button>
          ) : overdue ? (
          ...
          {canManage && (
            <>
              <button onClick={onGrade} className="btn-secondary btn-sm !px-2.5" title="Grade Quiz"><GraduationCap size={14} /></button>
              <button onClick={onEdit} className="btn-ghost btn-sm !px-2" title="Edit"><Pencil size={14} /></button>
              <button onClick={onDelete} className="btn-ghost btn-sm !px-2 text-brand-600 hover:bg-brand-50" title="Delete"><Trash2 size={14} /></button>
            </>
          )}
        </div>
      </div>
...
// Bug 3 fix: section change by user clears sessionId; populate does not
// ─────────────────────────────────────────────────────────────
...
export function QuizFormModal({ open, onClose, onSaved, quiz }: QuizFormModalProps) {
  ...
  // Only clear sessionId when the user explicitly changes section, not on populate
  const sectionChangedByUser = useRef(false);

  // Sessions filtered to active committee + selected section
  ...
  useEffect(() => {
    if (!open) return;
    sectionChangedByUser.current = false;
    if (quiz) {
      setTitle(quiz.title);
  ...
  const handleSectionChange = (v: string) => {
    sectionChangedByUser.current = true;
    setSectionId(v);
    setSessionId('');
  };
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      form_url: formUrl.trim() || null, start_time: startDatetime, deadline,
      committee_id: activeCommittee.id, session_id: sessionId,
  ...
  return (
    <Modal open={open} onClose={onClose} title={quiz ? 'Edit Quiz' : 'New Quiz'}
      footer={<>
      ...
      <div className="space-y-4">
        {activeCommittee && (
          <div className="flex items-center gap-2 px-3 h-9 rounded-xl bg-brand-50 border border-brand-200/60 text-sm">
            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: activeCommittee.color }} />
        ...
        <Field label="Description (optional)" value={desc} onChange={setDesc} placeholder="Brief description of this quiz" textarea />
        <Field label="Google Form URL" value={formUrl} onChange={setFormUrl} placeholder="https://forms.google.com/…" />

        {/* Section picker first — filters the sessions dropdown */}
        {sections.length > 0 && (
          <Field
        ...
        <div className="rounded-xl border border-ink-200 bg-ink-50 p-4 space-y-3">
          <p className="text-xs font-semibold text-ink-600 uppercase tracking-wide">Schedule</p>
          ...
          <p className="text-xs text-ink-400">The Google Form link will be locked until the opening date and time.</p>
        </div>
      </div>
    </Modal>
  );
...
function GradingModal({ quizId, onClose, onSaved }: { quizId: string; onClose: () => void; onSaved: () => void }) {
  const { push } = useToast();
  const { activeCommittee } = useAuth();
  ...
  useEffect(() => {
    ...
    for (const m of committeeMembers) {
      ...
      init[m.id] = { score: existing ? String(existing.score) : '', bonus: existing ? String(existing.bonus) : '' };
    }
    setEntries(init);
  ...
  const handleSave = async () => {
    ...
    if (dirtyIds.length === 0) { push('info', 'No changes to save'); onClose(); return; }
    setSaving(true);
    const rows = dirtyIds.map((mid) => {
      const e = entries[mid] ?? { score: '0', bonus: '0' };
      return { quiz_id: quizId, member_id: mid, score: Math.max(0, Number(e.score) || 0), bonus: Math.max(0, Number(e.bonus) || 0) };
    });
  ...
  return (
    <Modal open onClose={onClose} title={`Grade Quiz: ${quiz.title}`} width="max-w-2xl"
      footer={<>
        <button className="btn-secondary btn-md" onClick={onClose}>Cancel</button>
    ...
    >
      <div className="space-y-3 max-h-[62vh] overflow-y-auto pr-1">
        {committeeMembers.length === 0 && <p className="text-sm text-ink-500 text-center py-10">No members found in this committee.</p>}
            ...
            <div key={m.id} className="p-4 rounded-xl border border-ink-200/70 bg-ink-50/30">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min="0" max="100" value={e.score} onChange={(ev) => updateEntry(m.id, 'score', ev.target.value)} placeholder="0" className="input" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-ink-600 mb-1">Bonus</label>
                  <input type="number" min="0" value={e.bonus} onChange={(ev) => updateEntry(m.id, 'bonus', ev.target.value)} placeholder="0" className="input" />
                </div>
              </div>
            </div>
          );
