...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: callerMember } = await callerClient
      .from("members").select("role").eq("user_id", userData.user.id).maybeSingle();
    ...
    // Look up the auth user_id before deleting the member row
    const { data: member } = await adminClient
      .from("members").select("user_id").eq("id", member_id).maybeSingle();

    // Delete related records first (safe order)
    ...
    // Delete member row
    const { error: memberErr } = await adminClient.from("members").delete().eq("id", member_id);
    if (memberErr) {
